...
const ENHANCED_VOICE_STYLES = `
    ...
    #n8n-chat .chat-window {
        ...
        flex-direction: column;
        overflow: hidden;
        margin-bottom: 90px; /* Space for toggle button */
...
class EnhancedChatWrapper {
  ...
  enhanceChat() {
    ...
    console.log(
      ...
      Array.from(chatContainer.children).map((c) => c.className)
    );
  ...
  integrateVoiceButton(inputElement, sendButton) {
    ...
    if (
      inputParent === sendParent ||
      inputParent.contains(sendButton) ||
      sendParent.contains(inputElement)
    ) {
      const container = inputParent.contains(sendButton)
        ? inputParent
        : sendParent;
  ...
  addAlertMode(chatContainer) {
    ...
    const inputArea =
      chatContainer.querySelector('[class*="input"]') ||
      ...
      const observer = new MutationObserver((mutations) => {
        for (const mutation of mutations) {
          for (const node of mutation.addedNodes) {
            ...
            const isEmpty = emptyPatterns.some(
              (pattern) =>
            ...
            if (
              node.matches(
                ".enhanced-voice-message, .enhanced-play-button, .enhanced-notification, .enhanced-voice-button, .enhanced-input-container, button, audio"
              ) ||
    ...
    try {
      ...
      if (options.body) {
        ...
        if (typeof options.body === "string") {
            ...
            if (key === "message" || key === "text" || key === "content") {
              if (typeof value === "string") {
                try {
                  ...
                  messageText =
                    parsed.message || parsed.text || parsed.content || value;
                } catch (e) {
                  messageText = value;
                }
              } else {
          ...
          messageText =
            options.body.get("message") ||
            options.body.get("text") ||
            options.body.get("content") ||
            "";
        } else {
      ...
      if (messageText && messageText.trim()) {
        ...
        console.warn(
          "No message content found in request, falling back to original fetch"
        );
  ...
  async sendTextMessage(message) {
    try {
      ...
      if (
        ...
        !error.message.includes("Response with null body")
      ) {
  ...
  enhanceMessage(messageElement) {
    ...
    if (
      messageElement.matches(
        ".enhanced-voice-message, .enhanced-play-button, .enhanced-notification, .enhanced-voice-button, .enhanced-input-container, button, audio, .chat-message"
      )
    ) {
      return;
    }
  ...
  fixChatToggle(chatContainer) {
    ...
    console.log(
      "Chat container HTML:",
      chatContainer?.outerHTML?.substring(0, 500)
    );

    // Try multiple selectors for toggle button
    let toggleButton =
      chatContainer?.querySelector(".chat-window-toggle") ||
    ...
    let chatWindow =
      chatContainer?.querySelector(".chat-window") ||
    ...
    if (!chatWindow) {
      ...
      chatWindow =
        chatContainer?.querySelector('div[style*="flex"]') ||
  ...
  async sendVoiceMessage(audioBlob) {
    try {
      ...
      if (
        ...
        this.showNotification(
          `Voice message failed: ${error.message}`,
          "error"
        );
      }
    }
    ...
    try {
      ...
      selectors.forEach((selector) => {
        ...
        elements.forEach((element) => {
          ...
          if (isEmpty) {
            console.log(
              "Marking empty response for removal:",
              content.substring(0, 50)
            );
            ...
            console.log(
              "Removing essentially empty element:",
              content.substring(0, 50)
            );
            element.remove();
          }
  ...
  cleanupDuplicateMessages() {
    ...
    allMessages.forEach((message) => {
      ...
      const hasComments =
        message.innerHTML.includes("<!---->") &&
        !message.innerHTML.includes("enhanced");
      ...
      if (hasComments) {
        const enhancedMessages = allMessages.filter(
          (m) =>
            m !== message &&
            (m.classList.contains("enhanced-user") ||
              m.classList.contains("enhanced-bot")) &&
            m.textContent.trim() === content
        );

        if (enhancedMessages.length > 0) {
          console.log(
            "Removing duplicate original message:",
            content.substring(0, 50)
          );
          messagesToRemove.push(message);
        }
  ...
  removeStatusMessages() {
    ...
    allMessages.forEach((message) => {
      ...
      if (
        (hasComments && (isStatusMessage || isGenericError)) ||
        isStatusMessage
      ) {
        console.log(
          "Periodic cleanup: Removing unwanted message:",
          content.substring(0, 50)
        );
        message.remove();
      }
  ...
  injectOrderingStylesIntoShadowRootWithRetry(retries = 10) {
    ...
    if (retries > 0) {
      setTimeout(
        () => this.injectOrderingStylesIntoShadowRootWithRetry(retries - 1),
        300
      );
    } else {
  ...
  toggleAlertMode() {
    ...
    const textInput = document.querySelector(
      "#n8n-chat input, #n8n-chat textarea"
    );
